# Implementation of Pranav Rajpurkar, Awni Y. Hannun, Masoumeh Haghpanahi, Codie Bourn, Andrew Y. Ng
...
class ECGClassificationModel:
    ...
    def pad_depth(x, desired_channels):
        ...
        x = Conv1D(desired_channels, 1)(x)
        x = BatchNormalization()(x)
        return x

    def initial_block(self, input, k):
        x = Conv1D(filters=self.filters * k, kernel_size=self.kernel_size, padding='same')(input)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        return x
    ...
    def residual_block_type_1(self, input, k, res_id):
        ...
        x = Conv1D(filters=self.filters * k, kernel_size=self.kernel_size, padding='same')(x)
